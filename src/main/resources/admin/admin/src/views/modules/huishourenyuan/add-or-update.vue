<template>
	<div class="addEdit-block" :style='{"padding":"30px"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="回收工号" prop="huishougonghao">
					<el-input v-model="ruleForm.huishougonghao" placeholder="回收工号" clearable  :readonly="ro.huishougonghao"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' v-else class="input" label="回收工号" prop="huishougonghao">
					<el-input v-model="ruleForm.huishougonghao" placeholder="回收工号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="员工姓名" prop="yuangongxingming">
					<el-input v-model="ruleForm.yuangongxingming" placeholder="员工姓名" clearable  :readonly="ro.yuangongxingming"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0","display":"inline-block"}' v-else class="input" label="员工姓名" prop="yuangongxingming">
...
export default {
	data() {
		...
		return {
			...
			rules: {
				huishougonghao: [
					{ required: true, message: '回收工号不能为空', trigger: 'blur' },
				],
				mima: [
				],
				yuangongxingming: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `huishourenyuan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.huishourenyuanCrossAddOrUpdateFlag = false;
						 this.parent.search();
