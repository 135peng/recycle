			...
			<view class="uni-product-list" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","margin":"0 0 32rpx","backgroundColor":"#fff","flexWrap":"wrap","display":"flex","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"10"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discussjifenshangpin','修改')) || (!userid && isAuthFront('discussjifenshangpin','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('discussjifenshangpin','删除')) || (!userid && isAuthFront('discussjifenshangpin','删除'))" @click.stop="onDeleteTap(product.id)">
...
<script>
	export default {
        ...
        params['order'] = 'desc';
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`discussjifenshangpin`, params);
                } else {
                    res = await this.$api.list(`discussjifenshangpin`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('discussjifenshangpin', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`discussjifenshangpin`, searchForm);
                } else {
                    res = await this.$api.list(`discussjifenshangpin`, searchForm);
                }
				// 如果是第一页数据置空
