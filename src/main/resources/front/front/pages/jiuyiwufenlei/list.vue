		...
		<view :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
			<view class="cu-bar bg-white search" style="width:100%" :style="[{top:CustomBar + 'px'}]">
				<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
					...
					<view v-if="queryIndex==0" class="search-form round">
						...
						<input v-model="searchForm.jiuyiwufenlei" type="text" placeholder="旧衣物分类" ></input>
					</view>
				<view class="action">
			...
			<view class="uni-product-list" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","margin":"0 0 32rpx","backgroundColor":"#fff","flexWrap":"wrap","display":"flex","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
					<image :style='{"width":"100%","padding":"0","margin":"0","objectFit":"cover","display":"block","height":"300rpx"}' mode="aspectFill" class="uni-product-image" v-if="preHttp(product.image)" :src="product.image.split(',')[0]"></image>
					<image :style='{"width":"100%","padding":"0","margin":"0","objectFit":"cover","display":"block","height":"300rpx"}' mode="aspectFill" class="uni-product-image" v-else :src="product.image?baseUrl+product.image.split(',')[0]:''"></image>
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"10"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiuyiwufenlei','修改')) || (!userid && isAuthFront('jiuyiwufenlei','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiuyiwufenlei','删除')) || (!userid && isAuthFront('jiuyiwufenlei','删除'))" @click.stop="onDeleteTap(product.id)">
...
<script>
	export default {
        ...
        params['order'] = 'desc';
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`jiuyiwufenlei`, params);
                } else {
                    res = await this.$api.list(`jiuyiwufenlei`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jiuyiwufenlei', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`jiuyiwufenlei`, searchForm);
                } else {
                    res = await this.$api.list(`jiuyiwufenlei`, searchForm);
                }
				// 如果是第一页数据置空
