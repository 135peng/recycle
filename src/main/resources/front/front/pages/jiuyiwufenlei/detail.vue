	...
	<view class="content">
		<view class="container" :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
				...
				<view class="detail-list-item" :style='{"margin":"0 0 24rpx 0","borderColor":"#ccc","borderWidth":"0 0 4rpx 0","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}'>
					<view class="lable" :style='{"padding":"0 20rpx 0 0","color":"#333","textAlign":"center","background":"0","width":"160rpx","lineHeight":"80rpx","fontSize":"28rpx"}'>旧衣物分类：</view>
	...
	export default {
		...
		async onShow(options) {
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('jiuyiwufenlei', this.id);
		...
		methods: {
			...
			async init(){
				...
				let res = await this.$api.info('jiuyiwufenlei', this.id);
				this.detail = res.data;
				// 轮播图片
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussjiuyiwufenlei/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
