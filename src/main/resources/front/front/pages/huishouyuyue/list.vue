		...
		<view :style='{"width":"100%","padding":"0","position":"relative","background":"#fff","height":"100%"}'>
			<view class="cu-bar bg-white search" style="width:100%" :style="[{top:CustomBar + 'px'}]">
				<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
					...
					<view v-if="queryIndex==2" class="search-form round">
						<text class="cuIcon-search"></text>
						<input v-model="searchForm.huishouzhuangtai" type="text" placeholder="回收状态" ></input>
					</view>
				<view class="action">
			...
			<view class="uni-product-list" :style='{"padding":"24rpx","margin":"0","flexWrap":"wrap","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="uni-product" :style='{"boxShadow":"0 2rpx 12rpx rgba(0,0,0,.3)","margin":"0 0 32rpx","backgroundColor":"#fff","flexWrap":"wrap","display":"flex","width":"48%","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","color":"#333","textAlign":"center","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","order":"2"}'>{{product.yifushuliang}}</view>
					<view class="uni-product-title" :style='{"padding":"4rpx 20rpx","color":"#333","textAlign":"center","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","order":"2"}'>回收状态:{{product.huishouzhuangtai}}</view>
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto","order":"10"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('huishouyuyue','修改')) || (!userid && isAuthFront('huishouyuyue','修改'))" @click.stop="onUpdateTap(product.id)">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('huishouyuyue','删除')) || (!userid && isAuthFront('huishouyuyue','删除'))" @click.stop="onDeleteTap(product.id)">
...
<script>
	export default {
        ...
        params['order'] = 'desc';
				...
				if(this.searchForm.huishouzhuangtai){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`huishouyuyue`, params);
                } else {
                    params['sfsh'] = '是';
                    res = await this.$api.list(`huishouyuyue`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('huishouyuyue', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.huishouzhuangtai){
					searchForm['huishouzhuangtai'] = '%' + this.searchForm.huishouzhuangtai + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`huishouyuyue`, searchForm);
                } else {
                    res = await this.$api.list(`huishouyuyue`, searchForm);
                }
				// 如果是第一页数据置空
